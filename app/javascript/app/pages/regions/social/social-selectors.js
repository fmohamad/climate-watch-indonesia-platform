...
const getIndicatorSocial = createSelector(
  getIndicator,
  indicators => {
...
const getIndicatorOptions = createSelector(
  ...
  (model, metadata) => {
    ...
    let indicators = metadata.indicators.map(o => (
      {
        label: o.name,
        value: o.code,
        unit: o.unit,
...
const getDistrictOptions = createSelector(
  getMetadata,
  metadata => {
...
const getDefaults = createSelector(
  [getSelectedModel, getFilterOptions, getAllSelectedOption, getDefaultIndicator],
  (model, options, allSelectedOption, defaultIndicator) => ({
    model: get(options, 'model[0]'),
    indicator: model.value === 'education' ? defaultIndicator[0] : defaultIndicator[1],
...
const getChartData = createSelector(
  ...
  (model, indicators, options) => {
    ...
    const selectedDist = options.district.value === ALL_SELECTED ? provinceISO : options.district.value
    ...
    const filteredIndicators = filter(selectedIndicators, function(o) {
      return (o.indicator_code === selectedInd && o.location_iso_code3 === selectedDist)
    })
